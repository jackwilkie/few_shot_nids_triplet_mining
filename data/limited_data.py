...
def get_limited_train_set(
    ...
    for c in np.unique(y_data):
        ...
        num_samples = (
            num_samples or class_size
        )  # get whole class if num samples is None
        num_samples = (
            num_samples if num_samples > 1 else (class_size * num_samples) // 1
        )  # use num samples as ratio if less than one
        num_class_samples[c] = (
            num_samples  # store number of samples to samples from class
        )
    ...
    for i in range(set_num + 1):
        ...
        for c in np.unique(y_data):
            class_indicies = np.argwhere(
                y_pool == c
            ).flatten()  # get indicies of samples of current class
            ...
            if num_class_samples[c] > class_size:  # need more samples than class size
                if replacement:
                    ...
                    resampled_indicies = rng_choice(
                        class_indicies.tolist(),
                        extra_samples,
                        replace=True,
                        generator=rng,
                    )
...
def iterate_limited_train_set(
    x_data, y_data, benign_samples=0, attack_samples=0, seed=385929325, sets=20
):
    """
    Get training and testing data sets to train and evaluate models under limited data conditions. Gets desired number of samples per class
    in training set with the remainder of data being used for testing.
    ...
    for i in range(sets):
        ...
        for c in np.unique(y_data):
            class_indicies = (
                np.argwhere(y_pool == c).flatten().tolist()
            )  # get indicies of samples of current class

            # print(i)

            class_train_indicies = np.array(
                rng.sample(class_indicies, num_class_samples[c])
            )  # sample class indicie for trianing set
...
def cv_generator(
    x_data: Union[ndarray, Tensor],
    y_data: Union[ndarray, Tensor],
    fold: int,
    n_folds: int,
    seed: Optional[int] = None,
):
    if fold >= n_folds and n_folds != -1:
        raise ValueError(
            f"Requested fold number {fold}, greater than total number of folds: {n_folds}"
        )

    # -- convert data to numpy if tensors
    ...
    x_data, y_data = np.copy(x_data), np.copy(y_data)

    # -- get k fold cv data splits
    skf = StratifiedKFold(
        n_splits=n_folds,
        shuffle=True,
    ...
    for i_split, (train_indices, val_indices) in enumerate(skf.split(x_data, y_data)):
        if i_split == fold:
    ...
    raise ValueError(f"Fold {fold} was not found.")
