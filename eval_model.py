...
def parse_option():
    ...
    parser.add_argument(
        "--k", type=int, default=8, help="k value for KNN classifier during inference"
    )

    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="results/triplet_network/triplet_network.pt.tar",
        help="path to checkpoint file",
    )
    parser.add_argument(
        "--results_path",
        type=str,
        default="results/triplet_network/performance.json",
...
def triplet_network_eval(
    ...
    test_embeddings, test_labels = extract_features(
        model=model, x_data=x_test, y_data=y_test, batch=True, chunk_size=opt.chunk_size
    )
