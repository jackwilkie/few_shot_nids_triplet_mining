...
def knn_classifier_(
    ...
    for idx in range(0, num_test_samples, samples_per_chunk):
        ...
        ]  # use rest of test data if not enough for chunk
        batch_size = features.size(0)

        dists = dist_fn(
            features, x_train
        )  # Returns B x T, T is number of train samples
        ...
        distances_transform = (
            weight_fn(distances.clone(), temp=temp)
            if weight_fn is not None
            else distances.clone()
        )

        canidates = y_train.view(1, -1).expand(
            batch_size, -1
        )  # size is B x T. repeats y_train for each test sample
        retrieved_neighbours = T.gather(
            canidates, 1, indicies
        )  # B x k. Contains labels of nearest neighbours to each test sample

        retrieval_one_hot.resize_(
            batch_size * k, num_classes
        ).zero_()  # get zero tensor size B*k x C, C is number of classes
        retrieval_one_hot.scatter_(
            1, retrieved_neighbours.view(-1, 1), 1
        )  # one hot encoder neighbour class for each neighbour for each sample, size is B*k x C

        # calculate scores as e^d/t, where d is distance and t is temp
        dists = T.mul(
            retrieval_one_hot.view(batch_size, -1, num_classes),
            distances_transform.view(batch_size, -1, 1),
        )  # B x k x C, one hot distance of each neighbour
        probs = T.sum(
            dists, 1
        )  # B x C, summed distance of each test sample to each class
        y_pred = T.argmax(
            probs, dim=1
        )  # get class predictions as class with highest dist (similarity)

        # store predictions
        dist_list.append(probs.cpu().detach().numpy())  # store dists as ndarray
        class_list.append(
            y_pred.cpu().detach().numpy()
        )  # get class predictions and store as ndarray
...
def fit_knn(
    ...
    for k in k_vals:
        distances, indicies = dists.topk(
            k, largest=True, sorted=True
        )  # get neighbours and their distances
        ...
        for t_n, t in enumerate(temp_vals):
            for weight_fn_str in weight_fn:
                ...
                distances_transform = (
                    weight_fn_(distances.clone(), temp=t)
                    if weight_fn is not None
                    else distances.clone()
                )
                ...
                y_pred = (
                    T.argmax(probs, dim=1).cpu().detach().numpy()
                )  # get class predictions as class with highest dist (similarity)
