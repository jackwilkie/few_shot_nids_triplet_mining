...
def _get_triplet_mask(labels: T.Tensor) -> T.Tensor:
    ...
    I_eq = T.eye(
        labels.shape[0], dtype=T.bool, device=labels.device
    )  # get matrix where indices are equal (BxB boolean identity matrix)
    ...
    distinct_mask = T.logical_and(
        T.logical_and(z_neq_r, z_neq_c), r_neq_c
    )  # get mask showing true for triplets with 2 unique samples

    # Check if labels[i] == labels[j] and labels[i] != labels[k]
    label_eq = T.eq(
        T.unsqueeze(labels, 0), T.unsqueeze(labels, 1)
    )  # get 2d matric of showing location of equal labels
    ...
    label_mask = T.logical_and(
        z_eq_r, T.logical_not(z_eq_c)
    )  # create 3d mask of triplets with valid label combinations
...
class BatchAllTripletLoss(OnlineTripletMining):
    ...
    def forward(self, embeddings: T.Tensor, target: T.Tensor):
        ...
        triplet_loss = T.clamp(
            triplet_loss, min=0.0
        ...
        self.fraction_positive_triplets = num_positive_triplets / (
...
class BatchAllSemiHardTripletLoss(OnlineTripletMining):
    ...
    def forward(self, embeddings: T.Tensor, target: T.Tensor):
        ...
        triplet_loss = (
            triplet_loss * mask_triplet * mask_sh
        )  # apply masks to filter invalid triplets
        triplet_loss = T.mean(
            T.clamp(triplet_loss, min=0.0)
        )  # remove negative losses (easy triplets) and find mean loss
...
class BatchSemiHardTripletLoss(OnlineTripletMining):
    ...
    def forward(self, embeddings: T.Tensor, target: T.Tensor):
        ...
        triplet_loss = T.amax(
            triplet_loss, dim=(1, 2)
        )  # get hardest semihard triplet for each anchor
...
class BatchHardTripletLoss(OnlineTripletMining):
    ...
    def forward(self, embeddings: T.Tensor, target: T.Tensor):
        ...
        mask_ap = _get_anchor_positive_triplet_mask(
            target
        ).float()  # get mask for positive samples for each anchor

        # print(d_matrix)
        # print(mask_ap)
        d_ap = (
            mask_ap * d_matrix
        )  # zero distance for samples with labels which do not match anchor
        d_hard_p = (
            T.max(d_ap, dim=1, keepdim=True).values
        )  # get maximum distance between anchor and positive samples, for each anchor

        # Get hardest negative sample for each anchor
        mask_an = _get_anchor_negative_triplet_mask(target).float()
        d_max_an = T.max(
            d_matrix, dim=1, keepdim=True
        ).values  # get max distance betweene each anchor and negative sample
        d_an = (
            d_matrix + (d_max_an * (1.0 - mask_an))
        )  # add max distance to invalid pairs to ensure they are not selected for triplet (cannot 0 as they would be selected)

        d_hard_n = T.min(d_an, dim=1, keepdim=True).values

        triplet_loss = T.clamp(
            d_hard_p - d_hard_n + self.m, min=0.0
        )  # calculate triplet loss using hardest samples for each anchor
...
class TripletLossWrapper(SupervisedLoss):
    ...
    def __init__(
        self, loss, *args, online_probe: bool = True, batch_aug=Identity(), **kwargs
    ):
    ...
    def forward(self, model, x: T.Tensor, y: T.Tensor, mixed_precision, training):
        ...
        if self.online_probe:
            ...
            if self.cache_labels_:
                self.cache_labels(
                    logits_ce, y_true.to(logits.device), training=model.training
                )
