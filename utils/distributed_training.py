...
def setup(
    ...
    '''
    if world_size > 0:
        os.environ['MASTER_ADDR'] = '127.0.0.1'  # Set to the IP address of the master node
...
def is_dist_avail_and_initalised() -> bool:
    return True if dist.is_available() and dist.is_initialized() else False  # checks if code is currently being ran distributed

def get_rank() -> int:
    return 0 if not is_dist_avail_and_initalised() else dist.get_rank()  # get rank of current process
