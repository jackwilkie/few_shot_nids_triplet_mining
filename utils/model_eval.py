...
def model_eval(
    ...
    precision_scores = [(cm[i][i]/np.sum(cm[:,i]) if np.sum(cm[:,i]) > 0.0 else 0.0) for i in range(len(cm))]  # get precision for each class
    recall_scores = [(cm[i][i]/np.sum(cm[i]) if np.sum(cm[i]) > 0 else 0.0) for i in range(len(cm))]  #calculate recall value for each class
    ...
    results_dict[f'{label}Macro_F1'] = f1_score(y_true, y_pred, average="macro", zero_division = 0.0)
    results_dict[f'{label}Micro_F1'] = f1_score(y_true, y_pred, average="micro", zero_division = 0.0)
    results_dict[f'{label}Weighted_F1'] = f1_score(y_true, y_pred, average="weighted", zero_division = 0.0)
...
def get_metric_dict(y_true, y_pred) -> dict:
    ...
    #calculate F1 score using different averaging
    results_dict['Macro_F1'] = f1_score(y_true, y_pred, average="macro", zero_division = 0.0)
    results_dict['Micro_F1'] = f1_score(y_true, y_pred, average="micro", zero_division = 0.0)
    results_dict['Weighted_F1'] = f1_score(y_true, y_pred, average="weighted", zero_division = 0.0)
    
    results_dict['fp_rate'] = 1 - results_dict['Class_0_Recall']
    
    class_dr = np.array([np.count_nonzero(y_pred[y_true == class_num] ==class_num)/np.count_nonzero(y_true == class_num) for class_num in np.unique(y_true) if class_num != 0])
...
def balanced_auroc(scores, labels, return_class_level: bool = False, include_lower_thres: bool = False) -> float:
    """
    ...
    """
    class_auroc = mean_auroc(scores = scores, y_true = labels, return_class_level= True, include_lower_thres= include_lower_thres)
    class_auroc = [max(x,1 - x) for x in class_auroc[:-1]] if include_lower_thres else class_auroc
